...
**iOS is planned for the future, not launch.** This is the other reason
Capacitor was chosen over an Android-only approach (e.g. plain WebView
wrapping): Capacitor is cross-platform by design — the same React build
that produces the Android app also produces an iOS app via
`npx cap add ios`, with no separate codebase or rewrite. Practically,
this means:
- Every native-feature integration (camera, file system, push
  notifications) must go through **Capacitor's plugin API**, never a
  raw Android-only library — an Android-specific native module would
  have to be re-done for iOS later.
- Test camera/upload/offline-storage behavior against Capacitor's
  documented cross-platform behavior, not just against however Android
  happens to behave, since iOS has different permission prompts and
  background-execution limits (e.g. Safari/WebKit's stricter
  background sync and storage-eviction rules affect the offline-sync
  design in Section 5).
- No iOS-specific work is needed now — this is a "build it the
  cross-platform way from day one" discipline, not a parallel
  workstream. The actual iOS build (Apple Developer account, App Store
  submission, Xcode signing) is deferred until explicitly prioritized.
...
arkworkers-web/           (React PWA, wrapped by Capacitor for
                           Android now, iOS later — same build for both)
  src/
    components/
    screens/
  ...
  ios/                    (added later via `npx cap add ios` —
                           not created at launch, but the codebase above
                           must not assume Android-only behavior)
```
...
[ ] Timing: when to actually prioritize the iOS build (Apple Developer
    account cost/setup, App Store review process) — not urgent, but
    worth a target date once Android is stable
```
